...
const TokenTable: React.FC<TokenTableProps> = ({ tokens, isConnected }) => {
  const { openConnectModal } = useConnectModal();
  const [searchTerm, setSearchTerm] = useState('');
  const [sortBy, setSortBy] = useState('Newest');
  const [showSortOptions, setShowSortOptions] = useState(false);

  // Default tokens to display when no user tokens are available
  ...
  const formatTokensForDisplay = (tokenData: TokenData[]) => {
    return tokenData.map(token => {
      // Format created time if it's not already formatted
      let displayCreatedTime = token.createdTime;
      if (!displayCreatedTime && token.createdAt) {
        const now = new Date();
        const createdDate = new Date(token.createdAt);
        const diffMin = Math.floor((now.getTime() - createdDate.getTime()) / (1000 * 60));
        
        if (diffMin < 60) {
          displayCreatedTime = `${diffMin}m ago`;
        } else {
          const diffHours = Math.floor(diffMin / 60);
          displayCreatedTime = `${diffHours}h ago`;
        }
      }
      
      return {
        ...token,
        createdTime: displayCreatedTime || 'just now'
      };
    });
  };

  // Choose which tokens to display based on connection status and availability
  let displayTokens = tokens.length > 0 ? formatTokensForDisplay(tokens) : formatTokensForDisplay(defaultTokens);

  // Helper function to extract market cap value
  const getMarketCapValue = (marketCap: string | undefined): number => {
    if (!marketCap) return 0;
    return parseInt(marketCap.replace(/[$k]/g, '') || '0');
  };

  // Sort the tokens based on sortBy option
  displayTokens = displayTokens.sort((a, b) => {
    switch (sortBy) {
      case 'Newest':
        // Sort by created time (assuming newer items have "ago" values)
        return (a.createdTime?.includes('just now') ? -1 : 1) - (b.createdTime?.includes('just now') ? -1 : 1) || 
               (parseInt(a.createdTime?.split('m')[0] || '999') - parseInt(b.createdTime?.split('m')[0] || '999'));
      case 'Highest Cap':
        // Extract numerical value from market cap (removing $ and k)
  ...
  const DexScreenerIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fillRule="evenodd" viewBox="0 0 252 300" focusable="false" className="text-gray-500 dark:text-gray-400" fill="currentColor">
      <path d="M151.818 106.866c9.177-4.576 20.854-11.312 32.545-20.541 2.465 5.119 2.735 9.586 1.465 13.193-.9 2.542-2.596 4.753-4.826 6.512-2.415 1.901-5.431 3.285-8.765 4.033-6.326 1.425-13.712.593-20.419-3.197m1.591 46.886l12.148 7.017c-24.804 13.902-31.547 39.716-39.557 64.859-8.009-25.143-14.753-50.957-39.556-64.859l12.148-7.017a5.95 5.95 0 003.84-5.845c-1.113-23.547 5.245-33.96 13.821-40.498 3.076-2.342 6.434-3.518 9.747-3.518s6.671 1.176 9.748 3.518c8.576 6.538 14.934 16.951 13.821 40.498a5.95 5.95 0 003.84 5.845zM126 0c14.042.377 28.119 3.103 40.336 8.406 8.46 3.677 16.354 8.534 23.502 14.342 3.228 2.622 5.886 5.155 8.814 8.071 7.897.273 19.438-8.5 24.796-16.709-9.221 30.23-51.299 65.929-80.43 79.589-.012-.005-.02-.012-.029-.018-5.228-3.992-11.108-5.988-16.989-5.988s-11.76 1.996-16.988 5.988c-.009.005-.017.014-.029.018-29.132-13.66-71.209-49.359-80.43-79.589 5.357 8.209 16.898 16.982 24.795 16.709 2.929-2.915 5.587-5.449 8.814-8.071 7.148-5.808 15.042-10.665 23.502-14.342C97.882 3.103 111.959.377 126 0m-25.818 106.866c-9.176-4.576-20.854-11.312-32.544-20.541-2.465 5.119-2.735 9.586-1.466 13.193.901 2.542 2.597 4.753 4.826 6.512 2.416 1.901 5.432 3.285 8.766 4.033 6.326 1.425 13.711.593 20.418-3.197"></path>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            {showSortOptions && (
              <div className="absolute top-full left-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-10 w-full">
                {['Newest', 'Highest Cap', 'A-Z'].map((option) => (
                  <button
                    key={option}
      ...
      {!isConnected && (
        <div className="text-center p-8 bg-white/10 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-gray-700 mb-4">
          <p className="text-lg font-medium mb-4 dark:text-white">Connect your wallet to see your tokens</p>
          <button 
      ...
      {isConnected && (
        <div className="space-y-2">
          ...
          {filteredTokens.length > 0 ? (
            ...
            <div className="text-center p-6 bg-white/10 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-gray-700">
